...
# Syntax for seq - 1st argument is starting point, 2nd argument is ending 
# point, "by =" tells R the interval between each element

q2.vector <- seq(0, 50, by = 0.5)
...
# Syntax for quantile has 10th percentile = 0.1 

quantile(q2.vector, 0.1)

# 90th percentile = 0.9

quantile(q2.vector, 0.9)
...
# Before running the code below, make sure that you ran the code above to define
# q3.vector. We want our new vector to be the same length as q3.vector, with
# each element being a true when the corresponding element in q3.vector is
...
# Same length as q3.vector

length(q3.test)

# TRUE in table below counts number of elements that are greater than 2
...
# The q3.vector has 25 numeric ("num" in output below) elements:

str(q3.vector)

# Our q3.test vector has 25 logical ("logi" in output below) elements:
...
# wages for 7 people and missing values = NA's for 3 people). 
...
# The is.na() function will return a vector of the same length as q4.income 
# where each element of q4.missing indicates whether the corresponding element
# in q4.income is missing (TRUE if missing, FALSE if not)

q4.missing <- is.na(q4.income)
...
# to do with the NA's in the q4.income vector. We can calculate the average of 
...
# We can use a vector of numbers as an argument to a single quantile function
# call to get the 10th, 50th, and 90th percentiles: 

quantile(q4.income, c(0.1, 0.5, 0.9), na.rm = TRUE)
...
# Make sure that you upload your .R code file (and another file e.g., something
# HTML formatted). Remember that you don't need to submit anything for the Swirl
# lessons.
